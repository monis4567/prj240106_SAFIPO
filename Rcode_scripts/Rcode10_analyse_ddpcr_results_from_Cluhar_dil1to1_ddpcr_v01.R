...
wd05 <- "output09_ddpcr_results_Cluhar_dil1to1"
#paste dirs together
...
df_ddP$Target[grepl("admor",df_ddP$Target)] <- "Gadmor"
df_ddP$Target[grepl("eomel",df_ddP$Target)] <- "Neomel"
df_ddP$Target[grepl("lafle",df_ddP$Target)] <- "Plafle"
df_ddP$Target[grepl("lepla",df_ddP$Target)] <- "Plepla"
df_ddP$Target[grepl("yaare",df_ddP$Target)] <- "Myaare"
df_ddP$Target[grepl("luhar",df_ddP$Target)] <- "Cluhar"

# subset the data frame to only contain the 'Gadmor' target
# or subset the data frame to only contain the 'Plafle' target
#df_ddP <- df_ddP[(df_ddP$Target=="Gadmor"),]
#df_ddP <- df_ddP[(df_ddP$Target=="Plafle"),]
# df_ddP <- df_ddP[(df_ddP$Target=="Plepla"),]
# df_ddP <- df_ddP[grep("Plepla_dil1to1\\.csv",df_ddP$file),]
df_ddP <- df_ddP[(df_ddP$Target=="Cluhar"),]
df_ddP <- df_ddP[grep("Cluhar_dil1to1\\.csv",df_ddP$file),]
#df_ddP <- df_ddP[grep("Gadmor_dil1to100\\.csv",df_ddP$file),]
...
#st.f <- st.f[grepl("Gadmor_dil1to100",st.f)]
st.f <- st.f[grepl("Cluhar_dil1to1",st.f)]
...
unique(df_ddP$ddpcrNo)

# combine the  data frames using 'WellNumber' and 'ddpcrNo' columns
df_ddP02 <- df_ddP %>% dplyr::left_join(df_stp, 
                                        by=c("WellNumber","ddpcrNo"))

# unique(df_ddP02$file)
# View(df_ddP02)
...
# divide the 'ddpcr_copies_uL_per_L' column by 100 
# as the template added was 100 times diluted 
#df_ddP03$ddpcr_copies_uL_per_L <- df_ddP03$ddpcr_copies_uL_per_L/100
# divide the 'ddpcr_copies_uL_per_L' column by 1 
# as the template added was 1 times diluted 
df_ddP03$ddpcr_copies_uL_per_L <- df_ddP03$ddpcr_copies_uL_per_L/1
library(ggrepel)
...
df_ddP03$prv.type[grepl("NK",df_ddP03$Filt.mbr.pore)] <- "NK"
# check the copy count in the "3.00" filters
...
write.csv(df_ddP03, 
          file = paste0(wd00_out02,"/df_ddP03_Cluhar_dil1to1.csv") )
...
outfl <- paste0(wd00_wd05,"/Fig04b_copies_per_filtered_liter_compared_with_extract_v01_Cluhar_dil1to1.png")
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig04_copies_per_filtered_liter_v01_Cluhar_dil1to1.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
...
# divide the 'ddpcr_copies_uL_per_L' column by 100
# as the template added was 100 times diluted
# df_ddP03$ddpcr_copies_uL_per_L/100
# divide the 'ddpcr_copies_uL_per_L' column by 1
# as the template added was 1 times diluted
 df_ddP03$ddpcr_copies_uL_per_L/1
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig06_copies_per_uL_per_filtered_liter_v01_Cluhar_dil1to1.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
...
# divide the 'ddpcr_copies_uL_per_L' column by 100
# as the template added was 100 times diluted
#df_ddP03$copies_perLfiltered <- df_ddP03$copies_perLfiltered/100
# divide the 'ddpcr_copies_uL_per_L' column by 1
# as the template added was 1 times diluted
df_ddP03$copies_perLfiltered <- df_ddP03$copies_perLfiltered/1
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig07_eDNA_copies_per_filtered_liter_v01_Cluhar_dil1to1.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig07_eDNA_copies_per_filtered_liter_Cluhar_dil1to1_v02.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
...
max(df_ddP03$conc.copies.per.uL)
# subset to only comprise the 'PLC' filter type with 
# pore size 3.00
df_ddP03_PLC3 <- df_ddP03[(df_ddP03$filt.mbr=="PLC" & df_ddP03$filt.psz==3),]
# exclude any rows with NA in the 'conc.copies.per.uL' column
df_ddP03_PLC3 <- df_ddP03_PLC3[!is.na(df_ddP03_PLC3$conc.copies.per.uL),]

# check the max value in the 'conc.copies.per.uL' column
max(df_ddP03_PLC3$conc.copies.per.uL)
# check the min value in the 'conc.copies.per.uL' column
min(df_ddP03_PLC3$conc.copies.per.uL)
#
#
