...
wd05 <- "output06_ddpcr_results_Plafle_dil1to1"
#paste dirs together
...
df_ddP <- df_ddP[grep("Plafle_dil1to1\\.csv",df_ddP$file),]
...
st.f <- st.f[grepl("Plafle_dil1to1",st.f)]
...
df_ddP02 <- df_ddP %>% dplyr::left_join(df_stp, by=c("WellNumber","ddpcrNo"))
...
write.csv(df_ddP03, 
          file = paste0(wd00_out02,"/df_ddP03_Plafle_dil1to1.csv") )
...
outfl <- paste0(wd00_wd05,"/Fig04b_copies_per_filtered_liter_compared_with_extract_v01_Plafle_dil1to1.png")
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig04_copies_per_filtered_liter_v01_Plafle_dil1to1.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig06_copies_per_uL_per_filtered_liter_v01_Plafle_dil1to1.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
...
# define the output file name
outfl <- paste0(wd00_wd05,"/Fig07_eDNA_copies_per_filtered_liter_v01_Plafle_dil1to1.png")
#set variable to define if figures are to be saved
bSaveFigures<-T
# save the figure if the above 'bSaveFigures' is TRUE
if(bSaveFigures==T){
