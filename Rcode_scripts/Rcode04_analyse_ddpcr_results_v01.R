...
df_ddP03$ddpcr_copies_µL <-as.numeric(df_ddP03$`Conc(copies/µL)`)
# get copy number per uL per L of filtered water
df_ddP03$ddpcr_copies_µL_per_L <- df_ddP03$ddpcr_copies_µL/(df_ddP03$fltvol.mL/1000)
library(ggrepel)
...
df_ddP03$ddpcr_copies_µL_per_L[grepl("3.00",df_ddP03$Filt.mbr.pore2)]

# make a scatter plot
p <- ggplot(data = df_ddP03,
       aes(x = extrconc_ngpuL_Pfltvol.L, 
           y = ddpcr_copies_µL_per_L, 
           color= prv.type,
           fill = filt.pszc,
           shape = filt.mbr )) +
  geom_point(size=4) + 
...
df_ddP03$Copies_20uLWell <- as.numeric(df_ddP03$`Copies/20µLWell`)
...
df_ddP03$ddpcr_copies_µL_per_mL_per_min <- df_ddP03$ddpcr_copies_µL/df_ddP03$mlpermin
